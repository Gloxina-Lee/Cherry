...
// This fork is maintained independently. Ignore update offers for its parent theme.
function cherry_ignore_theme_updates($updates)
{
...
add_filter('site_transient_update_themes', 'cherry_ignore_theme_updates', PHP_INT_MAX);
add_filter('pre_set_site_transient_update_themes', 'cherry_ignore_theme_updates', PHP_INT_MAX);

function cherry_disable_theme_auto_update($update, $item)
{
    return isset($item->theme) && $item->theme === get_template() ? false : $update;
}
add_filter('auto_update_theme', 'cherry_disable_theme_auto_update', PHP_INT_MAX, 2);

// Remove jobs and PUC state left by earlier versions of this theme once per site.
function cherry_cleanup_legacy_updates()
{
    if (get_option('cherry_legacy_updates_removed')) {
        return;
    }

    $cron_cleanup_succeeded = true;
    if (!has_action('daily_event')) {
        $result = wp_clear_scheduled_hook('daily_event');
        $cron_cleanup_succeeded = $cron_cleanup_succeeded && !is_wp_error($result) && $result !== false;
    }
    $result = wp_clear_scheduled_hook('puc_cron_check_updates_theme-Sakurairo');
    $cron_cleanup_succeeded = $cron_cleanup_succeeded && !is_wp_error($result) && $result !== false;
    delete_site_option('puc_external_updates_theme-Sakurairo');
    if ($cron_cleanup_succeeded) {
        update_option('cherry_legacy_updates_removed', 1, false);
    }
}
add_action('init', 'cherry_cleanup_legacy_updates', 1);
